...
@section('meta_title', 'Factors & Multiples — Maths Game for Kids')
@section('meta_blurb', 'A free factors and multiples game for school kids — find factors, multiples, prime numbers, the highest common factor (HCF) and lowest common multiple (LCM).')
@section('meta_words', 'factors and multiples game, prime numbers game, hcf game, lcm game, primary school maths, learn factors and multiples')
...
@section('content')
    <div class="container-xl pb-2">
                ...
                <div class="card mult-setup-card">
                    <div class="card-body p-4">
                        ...
                        <div class="mb-4" id="typeToggles">
                            <button type="button" class="choice-toggle active" data-type="factor">Is it a factor?</button>
                            <button type="button" class="choice-toggle active" data-type="multiple">Is it a multiple?</button>
                            <button type="button" class="choice-toggle active" data-type="prime">Is it prime?</button>
                            <button type="button" class="choice-toggle active" data-type="hcf">Highest common factor</button>
                            <button type="button" class="choice-toggle active" data-type="lcm">Lowest common multiple</button>
                        </div>
                ...
                <div class="question-card-wrap mb-3">
                    <div class="card question-card">
                        <div class="card-body p-4 text-center">
                            <span class="target-badge" id="targetBadge">IS IT A FACTOR?</span>
                            <div class="fm-question" id="fmQuestion">Is 6 a factor of 24?</div>
                        </div>
                    </div>
        ...
        <div class="mult-wrap mt-5">
            <h2 class="h6 text-secondary text-uppercase mb-2">About this factors and multiples game</h2>
            <p class="text-secondary small">This free factors and multiples game gives kids practice with five key number skills: recognising factors and multiples, identifying prime numbers, and finding the highest common factor (HCF) and lowest common multiple (LCM) of two numbers. Choose which question types to include, set your question count and time limit, then work through as many as you can. These skills underpin fractions, ratio and algebra later on.</p>
        </div>
    </div>
@endsection
...
@push('scripts')
    <script>
        (function() {
            ...
            const fmQuestion = document.getElementById('fmQuestion');
            ...
            const ALL_TYPES = ['factor', 'multiple', 'prime', 'hcf', 'lcm'];
            ...
            function getHint(q) {
                if (q.type === 'factor') return 'Does ' + q.n + ' divide exactly by ' + q.candidate + ' with nothing left over?';
                if (q.type === 'multiple') return 'Does ' + q.candidate + ' divide exactly by ' + q.base + ' with nothing left over?';
                if (q.type === 'prime') return 'A prime number has exactly two factors: 1 and itself. Try dividing by 2, 3, 5 and 7.';
                if (q.type === 'hcf') return 'List the factors of both numbers, then find the biggest one they have in common.';
                return 'List the times tables of both numbers, then find the smallest number that appears in both.';
            }

            function formulaText(q) {
                if (q.type === 'factor') return q.n + ' ÷ ' + q.candidate + (q.answer === 'Yes' ? ' is exact, so yes.' : ' is not exact, so no.');
                if (q.type === 'multiple') return q.candidate + ' ÷ ' + q.base + (q.answer === 'Yes' ? ' is exact, so yes.' : ' is not exact, so no.');
                if (q.type === 'prime') return q.answer === 'Yes' ? q.n + ' has only two factors (1 and ' + q.n + '), so it is prime.' : q.n + ' has more than two factors, so it is not prime.';
                if (q.type === 'hcf') return 'HCF(' + q.a + ', ' + q.b + ') = ' + q.answer + '.';
                return 'LCM(' + q.a + ', ' + q.b + ') = ' + q.answer + '.';
            }

            function loadSettings() {
                try {
                    const raw = localStorage.getItem('factorsMultiplesGame.settings');
                    if (!raw) return;
            ...
            function saveSettings() {
                try {
                    localStorage.setItem('factorsMultiplesGame.settings', JSON.stringify({
            ...
            function buildQuestion(types) {
                ...
                if (type === 'factor') {
                    ...
                    // 50/50 ask about a true factor vs a plausible non-factor
                    const askTrue = Math.random() < 0.5;
                    let candidate;
                    if (askTrue) {
                        candidate = pickFrom(facts);
                    } else {
                        do { candidate = randInt(2, n - 1); } while (facts.includes(candidate));
                    }
                    const answer = facts.includes(candidate) ? 'Yes' : 'No';
                    return { type, n, candidate, answer, display: 'Is ' + candidate + ' a factor of ' + n + '?', badge: 'IS IT A FACTOR?' };
                }
                ...
                if (type === 'multiple') {
                    ...
                    let candidate;
                    if (askTrue) {
                        candidate = base * randInt(2, 10);
                    } else {
                        do { candidate = randInt(base * 2, base * 10); } while (candidate % base === 0);
                    }
                    const answer = (candidate % base === 0) ? 'Yes' : 'No';
                    return { type, base, candidate, answer, display: 'Is ' + candidate + ' a multiple of ' + base + '?', badge: 'IS IT A MULTIPLE?' };
                }

                if (type === 'prime') {
                    const n = randInt(2, 50);
                    const answer = isPrime(n) ? 'Yes' : 'No';
                    return { type, n, answer, display: 'Is ' + n + ' a prime number?', badge: 'IS IT PRIME?' };
                }
                ...
                if (type === 'hcf') {
                    ...
                    return { type, a, b, answer, display: 'What is the highest common factor (HCF) of ' + a + ' and ' + b + '?', badge: 'HIGHEST COMMON FACTOR' };
                }

                // lcm — keep numbers modest so the LCM doesn't get huge
                const a = randInt(2, 12);
                const b = randInt(2, 12);
                const answer = lcm(a, b);
                return { type, a, b, answer, display: 'What is the lowest common multiple (LCM) of ' + a + ' and ' + b + '?', badge: 'LOWEST COMMON MULTIPLE' };
            }
            ...
            function buildQuestions(types, count) {
                ...
                for (let i = 0; i < count; i++) {
                    ...
                    do {
                        ...
                        key = q.type + ':' + (q.n || '') + ':' + (q.candidate || '') + ':' + (q.a || '') + ':' + (q.b || '');
                        attempts++;
            ...
            function buildChoices(q) {
                if (q.type === 'factor' || q.type === 'multiple' || q.type === 'prime') {
                    const arr = ['Yes', 'No'];
                ...
                const deltas = [1, 2, -1, -2, 3, -3, 5, -5];

                if (q.type === 'lcm') {
                    const product = q.a * q.b;
                    if (product !== answer && product < answer * 6) choices.add(product);
                }
            ...
            function showQuestion() {
                ...
                fmQuestion.textContent = q.display;
            ...
            function showResults() {
                ...
                if (pct >= 0.9) message = "Amazing! You're a factors and multiples superstar!";
            ...
            loadSettings();
            if (selectedTypes.size === 0) {
                selectedTypes = new Set(['factor', 'multiple', 'prime', 'hcf', 'lcm']);
            }
