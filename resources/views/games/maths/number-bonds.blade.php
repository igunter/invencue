...
@section('meta_title', 'Number Bonds — Missing Number Game for Kids')
@section('meta_blurb', 'A free number bonds game for young kids — find the missing number needed to make 10, 20 or 100.')
@section('meta_words', 'number bonds game, missing number game, bonds to 10, bonds to 20, primary school maths, learn number bonds')
...
@section('content')
    <div class="container-xl pb-2">
        @include('partials.games-header', [
            'category' => $category,
            'game' => $game,
            'blurb' => "Pick your target number, then find what's missing!"
        ])
            ...
            <div id="setupScreen">
                <div class="card mult-setup-card">
                    <div class="card-body p-4">
                        <h2 class="h6 text-uppercase text-secondary mb-3"><i class="bi bi-bullseye"></i> Bonds to which number?</h2>

                        <div class="mb-2" id="targetToggles">
                            <button type="button" class="choice-toggle active" data-target="10">Bonds to 10</button>
                            <button type="button" class="choice-toggle active" data-target="20">Bonds to 20</button>
                            <button type="button" class="choice-toggle active" data-target="100">Bonds to 100</button>
                        </div>
                        ...
                        <div id="setupError" class="alert alert-warning mt-4 mb-0 d-none" role="alert">
                            Pick at least one target number to play with.
                        </div>
                    ...
                    <div class="card question-card">
                        <div class="card-body p-4 text-center">
                            <div class="d-flex align-items-center justify-content-center gap-3 mb-3">
                                ...
                                <span class="num-chip chip-b" id="missingChip">?</span>
                                <span class="op-symbol">=</span>
                                <span class="num-chip chip-a" id="targetChip">?</span>
                            </div>
                        </div>
                    </div>
        ...
        <div class="mult-wrap mt-5">
            <h2 class="h6 text-secondary text-uppercase mb-2">About this number bonds game</h2>
            <p class="text-secondary small">This free number bonds game helps young kids build the foundational skill of finding the missing number needed to reach 10, 20 or 100 — one of the earliest and most important building blocks in primary maths. Choose which target numbers to include, set your question count and time limit, then find as many missing numbers as you can. Strong number bonds make later addition, subtraction and mental maths much easier.</p>
        </div>
    </div>
@endsection
...
@push('scripts')
    <script>
        (function() {
            ...
            function getHint(q) {
                return 'Count on from ' + q.known + ' up to ' + q.target + ' — how many more do you need?';
            }

            function loadSettings() {
                try {
                    const raw = localStorage.getItem('numberBondsGame.settings');
                    if (!raw) return;
                    const saved = JSON.parse(raw);
                    if (Array.isArray(saved.targets)) {
                        saved.targets.forEach(function(t) {
                            if ([10, 20, 100].indexOf(t) !== -1) selectedTargets.add(t);
                        });
                    }
            ...
            function saveSettings() {
                try {
                    localStorage.setItem('numberBondsGame.settings', JSON.stringify({
            ...
            function renderToggles() {
                targetToggles.forEach(function(btn) {
                    ...
                    btn.classList.toggle('active', selectedTargets.has(t));
                });
            }
            ...
            targetToggles.forEach(function(btn) {
                btn.addEventListener('click', function() {
                    const t = parseInt(btn.dataset.target, 10);
                    if (selectedTargets.has(t)) {
                        selectedTargets.delete(t);
                    } else {
            ...
            function buildQuestion(targets) {
                const target = targets[randInt(0, targets.length - 1)];
                let known;
                if (target === 100) {
                    // keep bonds-to-100 restricted to multiples of 10 so it's approachable for young kids
                    known = randInt(0, 10) * 10;
                } else {
                    known = randInt(0, target);
                }
                const missing = target - known;
                return { target: target, known: known, missing: missing, answer: missing };
            }

            function buildQuestions(targets, count) {
                ...
                for (let i = 0; i < count; i++) {
                    ...
                    let attempts = 0;
                    do {
                        q = buildQuestion(targetList);
                        key = q.target + '-' + q.known;
                        attempts++;
            ...
            function buildChoices(q) {
                ...
                while (choices.size < 4 && idx < deltas.length) {
                    ...
                    if (candidate >= 0 && candidate <= q.target && candidate !== answer) choices.add(candidate);
                    idx++;
                }
                while (choices.size < 4) {
                    const candidate = answer + randInt(-12, 12);
                    if (candidate >= 0 && candidate <= q.target) choices.add(candidate);
                }
            ...
            function showQuestion() {
                ...
                factorA.textContent = q.known;
                targetChip.textContent = q.target;
                missingChip.textContent = '?';
            ...
            function handleAnswer(choice, btn) {
                ...
                if (isCorrect) {
                    ...
                    feedbackText.textContent = q.known + ' + ' + q.missing + ' = ' + q.target + '.';
            ...
            function handleTimeout() {
                ...
                feedbackBanner.classList.remove('d-none');
                feedbackText.textContent = "Time's up! " + q.known + ' + ' + q.missing + ' = ' + q.target + '.';
                feedbackBanner.classList.add('wrong-banner');
                playWrongSound();
                continueBtn.classList.remove('d-none');
            }
            ...
            function showResults() {
                ...
                if (pct >= 0.9) message = "Amazing! You're a number bonds superstar!";
            ...
            function startGame() {
                ...
                session = {
                    questions: buildQuestions(selectedTargets, settings.questionCount),
                    index: 0,
                    score: 0,
                    timerId: null,
                    timeLeft: 0,
