...
@section('meta_title', 'Probability — GCSE Maths Game')
@section('meta_blurb', 'A free probability game for GCSE-level practice — work out the probability of single and combined events, and probabilities that sum to 1.')
@section('meta_words', 'probability game, probability of an event, combined events, gcse maths game, learn probability')
...
@push('styles')
    <style>
        ...
        @media (max-width: 400px) {
            .prob-question { font-size: 1.25rem; }
        }
...
@section('content')
    <div class="container-xl pb-2">
        @include('partials.games-header', [
            'category' => $category,
            'game' => $game,
            'blurb' => 'Pick your question types, then work out the probability!'
        ])
                ...
                <div class="card mult-setup-card">
                    <div class="card-body p-4">
                        ...
                        <div class="mb-4" id="typeToggles">
                            <button type="button" class="choice-toggle active" data-type="single">Single event</button>
                            <button type="button" class="choice-toggle active" data-type="combined">Combined events</button>
                            <button type="button" class="choice-toggle active" data-type="complement">Probabilities sum to 1</button>
                        </div>
                ...
                <div class="question-card-wrap mb-3">
                    <div class="card question-card">
                        <div class="card-body p-4 text-center">
                            <span class="target-badge" id="targetBadge">SINGLE EVENT</span>
                            <div class="prob-question" id="probQuestion">A bag contains 3 red, 7 blue balls. What is the probability of picking a red ball?</div>
                        </div>
                    </div>
        ...
        <div class="mult-wrap mt-5">
            <h2 class="h6 text-secondary text-uppercase mb-2">About this probability game</h2>
            <p class="text-secondary small">This free probability game gives GCSE-level students practice working out the probability of single events, combined independent events, and probabilities that must sum to 1. Choose which question types to include, set your question count and time limit, then work through as many as you can, giving each answer as a fraction in its simplest form. It's a solid way to build fluency with probability notation and reasoning ahead of exams.</p>
        </div>
    </div>
@endsection
...
@push('scripts')
    <script>
        (function() {
            ...
            const probQuestion = document.getElementById('probQuestion');
            ...
            const ALL_TYPES = ['single', 'combined', 'complement'];
            const COLOURS = ['red', 'blue', 'green', 'yellow'];

            let selectedTypes = new Set();
            let settings = { questionCount: 10, timeLimit: 20 };
            let session = null; // { questions, index, score, timerId, timeLeft, correctText, answered }
            let paused = false;

            function getHint(q) {
                if (q.type === 'single') return 'Probability = number of favourable outcomes ÷ total number of outcomes.';
                if (q.type === 'combined') return 'For independent events, multiply the two probabilities together.';
                return 'All probabilities for an event add up to 1 — subtract the given probability from 1.';
            }

            function loadSettings() {
                try {
                    const raw = localStorage.getItem('probabilityGame.settings');
                    if (!raw) return;
            ...
            function saveSettings() {
                try {
                    localStorage.setItem('probabilityGame.settings', JSON.stringify({
            ...
            function buildQuestion(types) {
                ...
                if (type === 'single') {
                    // pick 2 or 3 colours with random counts 1-6 each, total kept reasonable
                    const numColours = randInt(2, 3);
                    const colours = COLOURS.slice(0, numColours);
                    const counts = colours.map(function() { return randInt(1, 6); });
                    const total = counts.reduce(function(a, b) { return a + b; }, 0);
                    const targetIdx = randInt(0, colours.length - 1);
                    const frac = simplifyFraction(counts[targetIdx], total);
                    const bagDesc = colours.map(function(c, i) { return counts[i] + ' ' + c; }).join(', ');
                    return {
                        type: type,
                        correctText: fracText(frac),
                        display: 'A bag contains ' + bagDesc + ' balls. What is the probability of picking a ' + colours[targetIdx] + ' ball?',
                        badge: 'SINGLE EVENT',
                        total: total, count: counts[targetIdx],
                    };
                }

                if (type === 'combined') {
                    // two independent single-digit-denominator events multiplied
                    const num1 = randInt(1, 4), den1 = randInt(num1 + 1, 6);
                    const num2 = randInt(1, 4), den2 = randInt(num2 + 1, 6);
                    const frac = simplifyFraction(num1 * num2, den1 * den2);
                    return {
                        type: type,
                        correctText: fracText(frac),
                        display: 'Event A has a probability of ' + num1 + '/' + den1 + '. Event B has a probability of ' + num2 + '/' + den2 + '. What is the probability of both A and B happening (independent events)?',
                        badge: 'COMBINED EVENTS',
                        num1: num1, den1: den1, num2: num2, den2: den2,
                    };
                }

                // complement — probabilities sum to 1
                const den = pickFrom([4, 5, 6, 8, 10, 12]);
                const num = randInt(1, den - 1);
                const compNum = den - num;
                const frac = simplifyFraction(compNum, den);
                return {
                    type: type,
                    correctText: fracText(frac),
                    display: 'The probability of an event happening is ' + num + '/' + den + '. What is the probability of it NOT happening?',
                    badge: 'PROBABILITIES SUM TO 1',
                    num: num, den: den,
                };
            }
            ...
            function buildQuestions(types, count) {
                ...
                for (let i = 0; i < count; i++) {
                    questions.push(buildQuestion(typeList));
                }
                return questions;
            }
            ...
            function buildChoices(q) {
                ...
                if (q.type === 'single') {
                    addCandidate(candidates, fracText(simplifyFraction(q.total - q.count, q.total)));
                    addCandidate(candidates, fracText(simplifyFraction(q.count, q.total + 1)));
                    addCandidate(candidates, fracText(simplifyFraction(q.count + 1, q.total)));
                    addCandidate(candidates, fracText(simplifyFraction(Math.max(q.count - 1, 1), q.total)));
                } else if (q.type === 'combined') {
                    addCandidate(candidates, fracText(simplifyFraction(q.num1 + q.num2, q.den1 + q.den2)));
                    addCandidate(candidates, fracText(simplifyFraction(q.num1 * q.num2, q.den1 + q.den2)));
                    addCandidate(candidates, fracText(simplifyFraction((q.num1 * q.num2) + 1, q.den1 * q.den2)));
                    addCandidate(candidates, fracText(simplifyFraction(q.num1 * q.num2, Math.max((q.den1 * q.den2) - 1, 1))));
                } else {
                    addCandidate(candidates, fracText(simplifyFraction(q.num, q.den)));
                    addCandidate(candidates, fracText(simplifyFraction(q.den - q.num + 1, q.den)));
                    addCandidate(candidates, fracText(simplifyFraction(Math.max(q.den - q.num - 1, 1), q.den)));
                    addCandidate(candidates, fracText(simplifyFraction(q.den - q.num, q.den + 1)));
                }
                ...
                while (choices.size < 4 && idx < candidates.length) {
                    ...
                    if (c !== correctText) choices.add(c);
                    idx++;
                }

                // Fallback: small random perturbations, simplified, until we have 4 unique fractions
                let guard = 0;
                while (choices.size < 4 && guard < 50) {
                    const num = randInt(1, 9);
                    const den = randInt(num + 1, 12);
                    const candidate = fracText(simplifyFraction(num, den));
            ...
            function showQuestion() {
                ...
                probQuestion.textContent = q.display;
            ...
            function wrongFeedback(q) {
                if (q.type === 'single') {
                    return q.count + '/' + q.total + ' = ' + q.correctText + '.';
                }
                if (q.type === 'combined') {
                    return q.num1 + '/' + q.den1 + ' × ' + q.num2 + '/' + q.den2 + ' = ' + q.correctText + '.';
                }
                return '1 − ' + q.num + '/' + q.den + ' = ' + q.correctText + '.';
            }
            ...
            function handleAnswer(choice, btn) {
                ...
                if (isCorrect) {
                    ...
                    feedbackText.textContent = wrongFeedback(q);
            ...
            function handleTimeout() {
                ...
                feedbackBanner.classList.remove('d-none');
                feedbackText.textContent = "Time's up! " + wrongFeedback(q);
                feedbackBanner.classList.add('wrong-banner');
                playWrongSound();
                continueBtn.classList.remove('d-none');
            }
            ...
            function showResults() {
                ...
                if (pct >= 0.9) message = "Amazing! You're a probability superstar!";
            ...
            loadSettings();
            if (selectedTypes.size === 0) {
                selectedTypes = new Set(['single', 'combined', 'complement']);
            }
