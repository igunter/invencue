...
@section('meta_title', 'Subtraction — Takeaway Game for Kids')
@section('meta_blurb', 'A free subtraction game for primary school kids — pick the numbers to subtract, how many questions, and how much time per question.')
@section('meta_words', 'subtraction game, takeaway game for kids, maths game, primary school maths, learn subtraction')
...
@push('styles')
    <style>
        ...
        .question-card {
            ...
            background: linear-gradient(160deg, #fff0f0, #fff5eb);
        ...
        .num-chip.chip-a { background: linear-gradient(135deg, #ff8787, #fa5252); }
        .num-chip.chip-b { background: linear-gradient(135deg, #868e96, #495057); }
...
@section('content')
    <div class="container-xl pb-2">
        @include('partials.games-header', [
            'category' => $category,
            'game' => $game,
            'blurb' => 'Pick your numbers, then answer as many takeaways as you can!'
        ])
            ...
            <div id="setupScreen">
                <div class="card mult-setup-card">
                    <div class="card-body p-4">
                        <h2 class="h6 text-uppercase text-secondary mb-3"><i class="bi bi-grid-3x3-gap"></i> Subtract which numbers?</h2>
                        ...
                        <div id="setupError" class="alert alert-warning mt-4 mb-0 d-none" role="alert">
                            Pick at least one number to play with.
                        </div>
        ...
        <div class="mult-wrap mt-5">
            <h2 class="h6 text-secondary text-uppercase mb-2">About this subtraction game</h2>
            <p class="text-secondary small">This free subtraction game gives primary school kids quick, repeatable practice at taking one number away from another. Pick the numbers to subtract, choose how many questions to answer and how much time you've got per question, then see how many takeaways you can get right. It's a handy way to build confidence with subtraction facts at home, in the classroom, or on the go &mdash; no sign-up required.</p>
        </div>
    </div>
@endsection
    ...
    <script>
        (function() {
            ...
            function loadSettings() {
                try {
                    const raw = localStorage.getItem('subtractionGame.settings');
                    if (!raw) return;
            ...
            function saveSettings() {
                try {
                    localStorage.setItem('subtractionGame.settings', JSON.stringify({
            ...
            pickLowBtn.addEventListener('click', function() {
                selectedTables = new Set([1, 2, 3, 4, 5, 6]);
                setupError.classList.add('d-none');
                renderToggles();
            });

            pickHighBtn.addEventListener('click', function() {
                selectedTables = new Set([7, 8, 9, 10, 11, 12]);
                setupError.classList.add('d-none');
                renderToggles();
            });
            ...
            function buildQuestions(tables, count) {
                ...
                for (let i = 0; i < count; i++) {
                    ...
                    let attempts = 0;
                    do {
                        b = tableList[randInt(0, tableList.length - 1)];
                        answer = randInt(0, 12);
                        key = b + '-' + answer;
                        attempts++;
                    } while (key === lastKey && attempts < 10);
                    lastKey = key;
                    questions.push({ a: b + answer, b: b, answer: answer });
                }
                return questions;
            }
            ...
            function renderVisualGroups(a, b) {
            ...
            function showResults() {
                ...
                if (pct >= 0.9) message = "Amazing! You're a subtraction superstar!";
            ...
            loadSettings();
            if (selectedTables.size === 0) {
                selectedTables = new Set([1, 2, 3, 4, 5]);
            }
