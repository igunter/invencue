...
@section('meta_title', 'Mixed Operations — Maths Game for Kids')
@section('meta_blurb', 'A free mixed maths game for primary school kids — practise adding, subtracting, multiplying and dividing all in one game.')
@section('meta_words', 'mixed maths game, arithmetic game for kids, maths game, primary school maths, addition subtraction multiplication division')
...
@push('styles')
    <style>
        ...
        .question-card {
            ...
            background: linear-gradient(160deg, #f8f5ff, #eef6ff);
        ...
        .visual-groups.size-md .visual-pile .visual-icon {
            font-size: 1rem;
        }
        ...
        .visual-groups.size-sm .visual-pile .visual-icon {
            font-size: 0.7rem;
        }
...
@section('content')
    <div class="container-xl pb-2">
        @include('partials.games-header', [
            'category' => $category,
            'game' => $game,
            'blurb' => 'Pick your numbers and operations, then answer as many questions as you can!'
        ])
            ...
            <div id="setupScreen">
                <div class="card mult-setup-card">
                    <div class="card-body p-4">
                        <h2 class="h6 text-uppercase text-secondary mb-3"><i class="bi bi-calculator"></i> Which operations?</h2>

                        <div class="mb-4" id="opToggles">
                            <button type="button" class="op-toggle active" data-op="multiply" title="Multiply">&times;</button>
                            <button type="button" class="op-toggle active" data-op="divide" title="Divide">&divide;</button>
                            <button type="button" class="op-toggle active" data-op="add" title="Add">+</button>
                            <button type="button" class="op-toggle active" data-op="subtract" title="Subtract">&minus;</button>
                        </div>
        ...
        <div class="mult-wrap mt-5">
            <h2 class="h6 text-secondary text-uppercase mb-2">About this mixed maths game</h2>
            <p class="text-secondary small">This free mixed operations game combines addition, subtraction, multiplication and division into one round, so kids get practice switching between the four basic operations &mdash; a skill real maths problems demand. Choose which operations to include, pick your numbers, set the question count and time limit, then see how many you can get right. It's a great way to test all-round arithmetic skills, not just one at a time.</p>
        </div>
    </div>
@endsection
...
@push('scripts')
    <script>
        (function() {
            ...
            const opToggles = Array.from(document.querySelectorAll('.op-toggle'));
            ...
            function getHint(q) {
                const hints = {
                    ...
                    multiply: 'Multiply the two numbers together.',
                    divide: 'Divide the first number by the second.',
                };
                return hints[q.op] || 'Work out the answer using the operation shown.';
            }

            function loadSettings() {
                try {
                    const raw = localStorage.getItem('mixedGame.settings');
                    if (!raw) return;
                    ...
                    if (Array.isArray(saved.ops)) {
                        saved.ops.forEach(function(op) {
                            if (OP_SYMBOLS[op]) selectedOps.add(op);
                        });
                    }
            ...
            function saveSettings() {
                try {
                    localStorage.setItem('mixedGame.settings', JSON.stringify({
            ...
            function generateQuestion(op, tableList) {
                if (op === 'multiply') {
                    const a = tableList[randInt(0, tableList.length - 1)];
                    const b = randInt(1, 12);
                    return { op: op, a: a, b: b, answer: a * b };
                }
                if (op === 'divide') {
                    const divisor = tableList[randInt(0, tableList.length - 1)];
                    const quotient = randInt(1, 12);
                    return { op: op, a: divisor * quotient, b: divisor, answer: quotient };
                }
                if (op === 'add') {
                    const a = tableList[randInt(0, tableList.length - 1)];
                    const b = randInt(1, 12);
                ...
                // subtract
                const b = tableList[randInt(0, tableList.length - 1)];
                const answer = randInt(0, 12);
                return { op: op, a: b + answer, b: b, answer: answer };
            }

            function buildQuestions(tables, ops, count) {
                ...
                for (let i = 0; i < count; i++) {
                    ...
                    do {
                        ...
                        q = generateQuestion(op, tableList);
                        key = op + ':' + q.a + ':' + q.b;
                        attempts++;
            ...
            function buildGroup(count, icon, extraClass) {
                const group = document.createElement('div');
                group.className = 'visual-group' + (extraClass ? ' ' + extraClass : '');
            ...
            function renderGroupsOf(groups, perGroup, icon) {
                ...
                for (let g = 0; g < groups; g++) {
                    ...
                    row.appendChild(buildGroup(perGroup, icon));
                }
            }

            function renderAdd(a, b, icon) {
                const row = document.createElement('div');
                row.className = 'visual-row';
                row.appendChild(buildGroup(a, icon));

                const plus = document.createElement('span');
                plus.className = 'visual-plus';
                plus.textContent = '+';
                row.appendChild(plus);

                row.appendChild(buildGroup(b, icon, 'group-b'));
                visualGroups.appendChild(row);
            }

            function renderSubtract(a, b, icon) {
                const remaining = a - b;
                const pile = document.createElement('div');
                pile.className = 'visual-pile';
                for (let n = 0; n < a; n++) {
                    const span = document.createElement('span');
                    span.className = 'visual-icon' + (n >= remaining ? ' removed' : '');
                    span.textContent = icon;
                    pile.appendChild(span);
                }
                visualGroups.appendChild(pile);
            }
            ...
            function renderVisual(q) {
                ...
                if (q.op === 'multiply') {
                    setSizeTier(q.a * q.b);
                    renderGroupsOf(q.a, q.b, icon);
                } else if (q.op === 'divide') {
                    setSizeTier(q.b * q.answer);
                    renderGroupsOf(q.b, q.answer, icon);
                } else if (q.op === 'add') {
                    setSizeTier(q.a + q.b);
                    renderAdd(q.a, q.b, icon);
                } else {
                    setSizeTier(q.a);
                    renderSubtract(q.a, q.b, icon);
                }
            }
            ...
            function showQuestion() {
                ...
                opSymbol.textContent = OP_SYMBOLS[q.op];
                renderVisual(q);
            ...
            function handleAnswer(choice, btn) {
                ...
                if (isCorrect) {
                    ...
                    feedbackText.textContent = q.a + ' ' + OP_SYMBOLS[q.op] + ' ' + q.b + ' = ' + q.answer;
            ...
            function handleTimeout() {
                ...
                feedbackBanner.classList.remove('d-none');
                feedbackText.textContent = "Time's up! " + q.a + ' ' + OP_SYMBOLS[q.op] + ' ' + q.b + ' = ' + q.answer;
                feedbackBanner.classList.add('wrong-banner');
                playWrongSound();
                continueBtn.classList.remove('d-none');
            }
            ...
            function showResults() {
                ...
                if (pct >= 0.9) message = "Amazing! You're a maths superstar!";
            ...
            function startGame() {
                if (selectedTables.size === 0 || selectedOps.size === 0) {
                ...
                session = {
                    questions: buildQuestions(selectedTables, selectedOps, settings.questionCount),
                    index: 0,
                    score: 0,
                    timerId: null,
                    timeLeft: 0,
