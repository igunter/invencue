...
@section('meta_title', 'Standard Form — GCSE Maths Game')
@section('meta_blurb', 'A free standard form game for GCSE-level practice — convert numbers to and from standard form, and multiply numbers written in standard form.')
@section('meta_words', 'standard form game, scientific notation game, convert standard form, gcse maths game, learn standard form')
...
@section('content')
    <div class="container-xl pb-2">
        @include('partials.games-header', [
            'category' => $category,
            'game' => $game,
            'blurb' => 'Pick your question types, then work out the answer!'
        ])
                ...
                <div class="card mult-setup-card">
                    <div class="card-body p-4">
                        ...
                        <div class="mb-4" id="typeToggles">
                            <button type="button" class="choice-toggle active" data-type="to">Convert to standard form</button>
                            <button type="button" class="choice-toggle active" data-type="from">Convert from standard form</button>
                            <button type="button" class="choice-toggle active" data-type="multiply">Multiply standard form numbers</button>
                        </div>
                ...
                <div class="question-card-wrap mb-3">
                    <div class="card question-card">
                        <div class="card-body p-4 text-center">
                            <span class="target-badge" id="targetBadge">CONVERT TO STANDARD FORM</span>
                            <div class="sf-question" id="sfQuestion">Write 400000 in standard form.</div>
                        </div>
                    </div>
        ...
        <div class="mult-wrap mt-5">
            <h2 class="h6 text-secondary text-uppercase mb-2">About this standard form game</h2>
            <p class="text-secondary small">This free standard form game gives GCSE-level students practice converting numbers to and from standard form (scientific notation), and multiplying numbers written in standard form. Choose which question types to include, set your question count and time limit, then work through as many as you can. It's a solid way to build fluency with standard form ahead of exams.</p>
        </div>
    </div>
@endsection
...
@push('scripts')
    <script>
        (function() {
            ...
            const sfQuestion = document.getElementById('sfQuestion');
            ...
            const ALL_TYPES = ['to', 'from', 'multiply'];
            ...
            function getHint(q) {
                if (q.type === 'to') return 'Move the decimal point until only one non-zero digit is in front of it — the number of places it moves is the power of 10.';
                if (q.type === 'from') return 'A positive power of 10 makes the number bigger (move the decimal point right); a negative power makes it smaller (move it left).';
                return 'Multiply the two coefficients together, and add the two powers of 10.';
            }

            function loadSettings() {
                try {
                    const raw = localStorage.getItem('standardFormGame.settings');
                    if (!raw) return;
            ...
            function saveSettings() {
                try {
                    localStorage.setItem('standardFormGame.settings', JSON.stringify({
            ...
            function randExponent() {
                // avoid n = 0 or 1 to keep "standard form" meaningfully different from the plain number
                const pool = [-6, -5, -4, -3, -2, 2, 3, 4, 5, 6, 7, 8];
                return pool[randInt(0, pool.length - 1)];
            }

            function formatStandardForm(a, n) {
                return a + ' × 10^' + n;
            }

            // Builds the plain-number text for a * 10^n where a is 1-9 (single digit).
            function formatPlainNumber(a, n) {
                if (n >= 0) {
                    return String(a) + '0'.repeat(n);
                }
                // n negative: 0.00...a with (-n - 1) zeros between the decimal point and a
                return '0.' + '0'.repeat(-n - 1) + a;
            }
            ...
            function buildQuestion(types) {
                ...
                if (type === 'to') {
                    const a = randInt(1, 9);
                    const n = randExponent();
                    const plain = formatPlainNumber(a, n);
                    const correctText = formatStandardForm(a, n);
                    return { type: type, a: a, n: n, display: 'Write ' + plain + ' in standard form.', correctText: correctText, badge: 'CONVERT TO STANDARD FORM' };
                }

                if (type === 'from') {
                    const a = randInt(1, 9);
                    const n = randExponent();
                    const correctText = formatPlainNumber(a, n);
                    return { type: type, a: a, n: n, display: 'Write ' + formatStandardForm(a, n) + ' as an ordinary number.', correctText: correctText, badge: 'CONVERT FROM STANDARD FORM' };
                }

                // multiply — keep a1*a2 < 10 so the result stays a clean single-digit coefficient with no renormalising
                let a1, a2;
                do {
                    a1 = randInt(1, 9);
                    a2 = randInt(1, 9);
                } while (a1 * a2 >= 10);
                const n1 = randExponent();
                const n2 = randExponent();
                const product = a1 * a2;
                const sumExp = n1 + n2;
                const correctText = formatStandardForm(product, sumExp);
                return {
                    type: type, a1: a1, a2: a2, n1: n1, n2: n2,
                    display: 'Work out (' + formatStandardForm(a1, n1) + ') × (' + formatStandardForm(a2, n2) + ').',
                    correctText: correctText,
                    badge: 'MULTIPLY STANDARD FORM',
                };
            }
            ...
            function buildChoices(q) {
                ...
                if (q.type === 'to' || q.type === 'multiply') {
                    const a = q.type === 'to' ? q.a : (q.a1 * q.a2);
                    const n = q.type === 'to' ? q.n : (q.n1 + q.n2);
                    const candidates = [];
                    candidates.push(formatStandardForm(a, n + 1));
                    candidates.push(formatStandardForm(a, n - 1));
                    if (a + 1 <= 9) candidates.push(formatStandardForm(a + 1, n));
                    if (a - 1 >= 1) candidates.push(formatStandardForm(a - 1, n));
                    candidates.push(formatStandardForm(a, n + 2));
                    candidates.push(formatStandardForm(a, n - 2));
                    ...
                    while (choices.size < 4 && guard < 30) {
                        const shift = randInt(-3, 3) || 1;
                        choices.add(formatStandardForm(a, n + shift));
                        guard++;
                    }
                } else {
                    // 'from': plain-number text answers — shift the decimal point, or nudge the digit
                    const a = q.a;
                    const n = q.n;
                    const candidates = [];
                    candidates.push(formatPlainNumber(a, n + 1));
                    candidates.push(formatPlainNumber(a, n - 1));
                    if (a + 1 <= 9) candidates.push(formatPlainNumber(a + 1, n));
                    if (a - 1 >= 1) candidates.push(formatPlainNumber(a - 1, n));
                    candidates.push(formatPlainNumber(a, n + 2));
                    candidates.push(formatPlainNumber(a, n - 2));
                    ...
                    while (choices.size < 4 && guard < 30) {
                        const shift = randInt(-3, 3) || 1;
                        choices.add(formatPlainNumber(a, n + shift));
                        guard++;
                    }
                }
            ...
            function showQuestion() {
                ...
                sfQuestion.textContent = q.display;
            ...
            function handleAnswer(choice, btn) {
                ...
                if (isCorrect) {
                    ...
                    feedbackText.textContent = q.display + ' Answer: ' + q.correctText;
            ...
            function handleTimeout() {
                ...
                feedbackBanner.classList.remove('d-none');
                feedbackText.textContent = "Time's up! " + q.display + ' Answer: ' + q.correctText;
                feedbackBanner.classList.add('wrong-banner');
                playWrongSound();
                continueBtn.classList.remove('d-none');
            }
            ...
            function showResults() {
                ...
                if (pct >= 0.9) message = "Amazing! You're a standard form superstar!";
            ...
            loadSettings();
            if (selectedTypes.size === 0) {
                selectedTypes = new Set(['to', 'from', 'multiply']);
            }
