...
@section('meta_title', 'Money & Coins — Counting Money Game for Kids')
@section('meta_blurb', 'A free money game for kids — count up UK coins to find the total, or work out the change from a purchase.')
@section('meta_words', 'money game for kids, counting coins, uk coins game, change game, primary school maths, learn money')
...
@push('styles')
    <style>
        ...
        @media (max-width: 400px) {
            .money-question { font-size: 1.25rem; }
        }
...
@section('content')
    <div class="container-xl pb-2">
        @include('partials.games-header', [
            'category' => $category,
            'game' => $game,
            'blurb' => 'Pick your question types, then count the money!'
        ])
                ...
                <div class="card mult-setup-card">
                    <div class="card-body p-4">
                        ...
                        <div class="mb-4" id="typeToggles">
                            <button type="button" class="choice-toggle active" data-type="total">Add up the coins</button>
                            <button type="button" class="choice-toggle active" data-type="change">Find the change</button>
                        </div>
                ...
                <div class="question-card-wrap mb-3">
                    <div class="card question-card">
                        <div class="card-body p-4 text-center">
                            <span class="target-badge" id="targetBadge">ADD UP THE COINS</span>
                            <div class="money-question" id="moneyQuestion">Add up these coins: 10p + 20p + 5p. What is the total?</div>
                        </div>
                    </div>
        ...
        <div class="mult-wrap mt-5">
            <h2 class="h6 text-secondary text-uppercase mb-2">About this money game</h2>
            <p class="text-secondary small">This free money game gives kids practice with real-world number skills — adding up a handful of UK coins to find a total, and working out the change from a purchase. Choose which question types to include, set your question count and time limit, then work through as many as you can. It's a practical way to build confidence with money before it matters for real.</p>
        </div>
    </div>
@endsection
...
@push('scripts')
    <script>
        (function() {
            ...
            const moneyQuestion = document.getElementById('moneyQuestion');
            ...
            function formatPence(p) {
                if (p >= 100) {
                    ...
                    const pence = p % 100;
                    return '£' + pounds + (pence > 0 ? '.' + (pence < 10 ? '0' + pence : pence) : '');
                }
                return p + 'p';
            }

            function getHint(q) {
                if (q.type === 'total') return 'Add the coins together, starting with the biggest one first.';
                return 'Change = the amount you paid − the price of the item.';
            }

            function loadSettings() {
                try {
                    const raw = localStorage.getItem('moneyCoinsGame.settings');
                    if (!raw) return;
            ...
            function saveSettings() {
                try {
                    localStorage.setItem('moneyCoinsGame.settings', JSON.stringify({
            ...
            function buildQuestion(types) {
                ...
                if (type === 'total') {
                    ...
                    // keep the pool to smaller/medium coins so totals stay approachable for kids
                    const pool = [1, 2, 5, 10, 20, 50, 100];
                    const coins = [];
                    let total = 0;
                    ...
                    const listText = coins.map(formatPence).join(' + ');
                    return {
                        type,
                        answerPence: total,
                        display: 'Add up these coins: ' + listText + '. What is the total?',
                        badge: 'ADD UP THE COINS',
                    };
                }

                // change — "you pay with a round note/coin, item costs a smaller round-ish amount"
                const priceOptions = [];
                for (let p = 5; p <= 950; p += 5) priceOptions.push(p); // 5p to £9.50 in 5p steps
                const price = pickFrom(priceOptions);
                const paymentOptions = [100, 200, 500, 1000].filter(function(p) { return p > price; });
                const payment = pickFrom(paymentOptions);
                const change = payment - price;
                return {
                    type,
                    answerPence: change,
                    display: 'An item costs ' + formatPence(price) + '. You pay with ' + formatPence(payment) + '. How much change do you get?',
                    badge: 'FIND THE CHANGE',
                    price, payment,
                };
            }
            ...
            function buildQuestions(types, count) {
                ...
                for (let i = 0; i < count; i++) {
                    ...
                    do {
                        ...
                        attempts++;
                    } while (q.type === lastType && typeList.length > 1 && attempts < 10);
                    lastType = q.type;
                    questions.push(q);
                }
                return questions;
            ...
            function buildChoices(q) {
                const answer = q.answerPence;
                const choices = new Set([answer]);

                const deltas = [5, 10, -5, -10, 20, -20, 50, -50];
                ...
                while (choices.size < 4) {
                    const candidate = answer + randInt(-50, 50);
                    if (candidate > 0 && candidate !== answer) choices.add(candidate);
                }
            ...
            function showQuestion() {
                ...
                moneyQuestion.textContent = q.display;
                ...
                choices.forEach(function(choice) {
                    ...
                    btn.textContent = formatPence(choice);
                    btn.dataset.pence = String(choice);
            ...
            function revealCorrect(correctAnswer, chosenBtn) {
                Array.from(answerGrid.querySelectorAll('.answer-btn')).forEach(function(btn) {
                    if (parseInt(btn.dataset.pence, 10) === correctAnswer) {
            ...
            function handleAnswer(choice, btn) {
                ...
                lockAnswers();
                const q = session.questions[session.index];
                const isCorrect = choice === q.answerPence;

                revealCorrect(q.answerPence, isCorrect ? null : btn);
                ...
                if (isCorrect) {
                    ...
                    if (q.type === 'total') {
                        feedbackText.textContent = 'The total is ' + formatPence(q.answerPence) + '.';
                    } else {
                        feedbackText.textContent = formatPence(q.payment) + ' − ' + formatPence(q.price) + ' = ' + formatPence(q.answerPence) + '.';
                    }
            ...
            function handleTimeout() {
                ...
                lockAnswers();
                const q = session.questions[session.index];
                revealCorrect(q.answerPence, null);
                feedbackBanner.classList.remove('d-none');
                if (q.type === 'total') {
                    feedbackText.textContent = "Time's up! The total is " + formatPence(q.answerPence) + '.';
                } else {
                    feedbackText.textContent = "Time's up! " + formatPence(q.payment) + ' − ' + formatPence(q.price) + ' = ' + formatPence(q.answerPence) + '.';
                }
                feedbackBanner.classList.add('wrong-banner');
                playWrongSound();
                continueBtn.classList.remove('d-none');
            ...
            function showResults() {
                ...
                if (pct >= 0.9) message = "Amazing! You're a money superstar!";
            ...
            loadSettings();
            if (selectedTypes.size === 0) {
                selectedTypes = new Set(['total', 'change']);
            }
