...
@section('meta_title', 'Skip Counting — Number Sequences Game for Kids')
@section('meta_blurb', 'A free skip counting game for kids — count on in 2s, 3s, 5s and 10s, and find the next number in a sequence.')
@section('meta_words', 'skip counting game, number sequence game, count in 2s 5s 10s, primary school maths, learn skip counting')
...
@section('content')
    <div class="container-xl pb-2">
        @include('partials.games-header', [
            'category' => $category,
            'game' => $game,
            'blurb' => 'Pick your step sizes, then find the next number!'
        ])
            ...
            <div id="setupScreen">
                <div class="card mult-setup-card">
                    <div class="card-body p-4">
                        <h2 class="h6 text-uppercase text-secondary mb-3"><i class="bi bi-list-check"></i> Which step sizes?</h2>
                        <div class="mb-4" id="stepToggles">
                            <button type="button" class="choice-toggle active" data-step="2">Count in 2s</button>
                            <button type="button" class="choice-toggle active" data-step="3">Count in 3s</button>
                            <button type="button" class="choice-toggle active" data-step="5">Count in 5s</button>
                            <button type="button" class="choice-toggle active" data-step="10">Count in 10s</button>
                        </div>
                        ...
                        <div id="setupError" class="alert alert-warning mt-4 mb-0 d-none" role="alert">
                            Pick at least one step size to play with.
                        </div>
                ...
                <div class="question-card-wrap mb-3">
                    <div class="card question-card">
                        <div class="card-body p-4 text-center">
                            <div class="equation-display" id="sequenceText">5, 10, 15, 20, ?</div>
                            <div class="text-secondary mt-2">What comes next?</div>
                        </div>
                    </div>
        ...
        <div class="mult-wrap mt-5">
            <h2 class="h6 text-secondary text-uppercase mb-2">About this skip counting game</h2>
            <p class="text-secondary small">This free skip counting game helps kids practise counting on in equal steps — 2s, 3s, 5s and 10s — by finding the next number in a sequence. Choose which step sizes to include, set your question count and time limit, then work through as many sequences as you can. It's a great warm-up skill that leads naturally into the times tables.</p>
        </div>
    </div>
@endsection
...
@push('scripts')
    <script>
        (function() {
            ...
            const stepToggles = Array.from(document.querySelectorAll('#stepToggles .choice-toggle'));
            ...
            function getHint(q) {
                return 'Add ' + q.step + ' to the last number in the sequence.';
            }

            function loadSettings() {
                try {
                    const raw = localStorage.getItem('skipCountingGame.settings');
                    if (!raw) return;
                    const saved = JSON.parse(raw);
                    if (Array.isArray(saved.steps)) {
                        saved.steps.forEach(function(s) {
                            const n = parseInt(s, 10);
                            if (n === 2 || n === 3 || n === 5 || n === 10) selectedSteps.add(n);
                        });
                    }
            ...
            function saveSettings() {
                try {
                    localStorage.setItem('skipCountingGame.settings', JSON.stringify({
            ...
            function renderToggles() {
                stepToggles.forEach(function(btn) {
                    const s = parseInt(btn.dataset.step, 10);
                    btn.classList.toggle('active', selectedSteps.has(s));
                });
            }
            ...
            // Builds one skip-counting sequence question for the given step sizes.
            // Returns a 4-number sequence plus the 5th number as the answer.
            function buildQuestion(steps) {
                const step = steps[randInt(0, steps.length - 1)];
                const startMultiplier = randInt(0, 8); // keeps starting numbers reasonably small
                const start = startMultiplier * step;
                const seq = [start, start + step, start + step * 2, start + step * 3];
                const answer = start + step * 4;
                const display = seq.join(', ') + ', ?';
                return { step: step, seq: seq, answer: answer, display: display };
            }

            function buildQuestions(steps, count) {
            ...
            function buildChoices(q) {
                ...
                const candidates = [
                    answer - step,      // stopped one step too early
                    answer + step,      // went one step too far
                    q.seq[3] + 1,        // off-by-one from the last shown number
                    answer - 1,
                    answer + 1,
                ];
                let idx = 0;
                ...
                while (choices.size < 4) {
                    const c = answer + randInt(-step * 2, step * 2);
                    if (c > 0 && c !== answer) choices.add(c);
                }
            ...
            function handleAnswer(choice, btn) {
                ...
                lockAnswers();
                const q = session.questions[session.index];
                const isCorrect = parseInt(choice, 10) === q.answer;
                ...
                if (isCorrect) {
                    ...
                    feedbackText.textContent = q.seq[q.seq.length - 1] + ' + ' + q.step + ' = ' + q.answer + '.';
            ...
            function handleTimeout() {
                ...
                feedbackBanner.classList.remove('d-none');
                feedbackText.textContent = "Time's up! " + q.seq[q.seq.length - 1] + ' + ' + q.step + ' = ' + q.answer + '.';
                feedbackBanner.classList.add('wrong-banner');
                playWrongSound();
                continueBtn.classList.remove('d-none');
            }
            ...
            function showResults() {
                ...
                if (pct >= 0.9) message = "Amazing! You're a skip counting superstar!";
            ...
            function startGame() {
                ...
                session = {
                    questions: buildQuestions(selectedSteps, settings.questionCount),
                    index: 0,
                    score: 0,
                    timerId: null,
                    timeLeft: 0,
