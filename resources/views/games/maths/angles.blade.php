...
@section('meta_title', 'Angles — Angle Types Game for Kids')
@section('meta_blurb', 'A free angles game for school kids — practise identifying acute, right, obtuse and reflex angles.')
@section('meta_words', 'angles game, angle types game for kids, acute obtuse reflex angle, maths game, primary school maths, learn angles')
...
@section('content')
    <div class="container-xl pb-2">
        @include('partials.games-header', [
            'category' => $category,
            'game' => $game,
            'blurb' => 'Pick your angle types, then say what kind of angle you can see!'
        ])
            ...
            <div id="setupScreen">
                <div class="card mult-setup-card">
                    <div class="card-body p-4">
                        <h2 class="h6 text-uppercase text-secondary mb-3"><i class="bi bi-triangle"></i> Which angle types?</h2>
                        <div class="mb-4" id="targetToggles">
                            <button type="button" class="choice-toggle active" data-type="acute">Acute</button>
                            <button type="button" class="choice-toggle active" data-type="right">Right</button>
                            <button type="button" class="choice-toggle active" data-type="obtuse">Obtuse</button>
                            <button type="button" class="choice-toggle active" data-type="reflex">Reflex</button>
                        </div>
                        ...
                        <div id="setupError" class="alert alert-warning mt-4 mb-0 d-none" role="alert">
                            Pick at least one angle type to play with.
                        </div>
                ...
                <div class="question-card-wrap mb-3">
                    <div class="card question-card">
                        <div class="card-body p-4 text-center">
                            <span class="target-badge">What type of angle is this?</span>
                            <div class="angle-svg-wrap" id="angleVisual"></div>
                        </div>
                    </div>
        ...
        <div class="mult-wrap mt-5">
            <h2 class="h6 text-secondary text-uppercase mb-2">About this angles game</h2>
            <p class="text-secondary small">This free angles game trains kids to recognise acute, right, obtuse and reflex angles at a glance &mdash; a core geometry skill for primary and early secondary maths. Pick which angle types to include, set your question count and time limit, then identify as many as you can. Spotting angle types quickly makes later topics like angle rules and shape properties much easier to pick up.</p>
        </div>
    </div>
@endsection
...
@push('scripts')
    <script>
        (function() {
            ...
            const typeToggles = Array.from(document.querySelectorAll('#targetToggles .choice-toggle'));
            ...
            const angleVisual = document.getElementById('angleVisual');
            ...
            const TYPE_LABELS = { acute: 'Acute', right: 'Right', obtuse: 'Obtuse', reflex: 'Reflex' };
            ...
            const ALL_TYPES = ['acute', 'right', 'obtuse', 'reflex'];
            ...
            function getHint() {
                return 'Acute is less than 90°, right is exactly 90°, obtuse is between 90° and 180°, and reflex is more than 180°.';
            }

            function loadSettings() {
                try {
                    const raw = localStorage.getItem('anglesGame.settings');
                    if (!raw) return;
                    const saved = JSON.parse(raw);
                    if (Array.isArray(saved.types)) {
                        saved.types.forEach(function(t) {
                            if (TYPE_LABELS[t]) selectedTypes.add(t);
                        });
                    }
            ...
            function saveSettings() {
                try {
                    localStorage.setItem('anglesGame.settings', JSON.stringify({
            ...
            function generateDegree(type) {
                if (type === 'acute') return randInt(10, 80);
                if (type === 'right') return 90;
                if (type === 'obtuse') return randInt(100, 170);
                return randInt(190, 350); // reflex
            }
            ...
            function buildQuestions(types, count) {
                ...
                for (let i = 0; i < count; i++) {
                    ...
                    let attempts = 0;
                    do {
                        type = typeList[randInt(0, typeList.length - 1)];
                        deg = generateDegree(type);
                        key = type + ':' + deg;
                        attempts++;
                    } while (key === lastKey && attempts < 10);
                    lastKey = key;
                    questions.push({ type: type, deg: deg });
                }
                return questions;
            }

            function describeArc(cx, cy, r, deg) {
                const startX = cx + r;
                const startY = cy;
            ...
            function renderAngle(q) {
                ...
                let marker = '';
                if (q.type === 'right') {
                    marker = '<rect x="' + cx + '" y="' + (cy - 18) + '" width="18" height="18" fill="none" stroke="#0d6efd" stroke-width="2" />';
                } else {
                    const arcPath = describeArc(cx, cy, 40, deg);
                    marker = '<path d="' + arcPath + '" fill="none" stroke="#0d6efd" stroke-width="3" />';
                }

                const svg =
                    '<svg viewBox="0 0 220 200" width="280">' +
                        '<line x1="' + cx + '" y1="' + cy + '" x2="' + (cx + rayLen) + '" y2="' + cy + '" stroke="#495057" stroke-width="4" stroke-linecap="round" />' +
                        '<line x1="' + cx + '" y1="' + cy + '" x2="' + x2 + '" y2="' + y2 + '" stroke="#495057" stroke-width="4" stroke-linecap="round" />' +
                        marker +
                        '<circle cx="' + cx + '" cy="' + cy + '" r="4" fill="#495057" />' +
                    '</svg>';

                angleVisual.innerHTML = svg;
            }
            ...
            function showQuestion() {
                ...
                choices.forEach(function(choice) {
                    ...
                    btn.textContent = TYPE_LABELS[choice];
            ...
            function revealCorrect(correctType, chosenBtn) {
                Array.from(answerGrid.querySelectorAll('.answer-btn')).forEach(function(btn) {
                    if (btn.dataset.value === correctType) {
            ...
            function describeAngle(q) {
                return 'This angle is ' + q.deg + '°, which makes it ' + (q.type === 'acute' ? 'an ' : 'a ') + TYPE_LABELS[q.type].toUpperCase() + ' angle (' + TYPE_DESCRIPTIONS[q.type] + ').';
            }
            ...
            function handleAnswer(choice, btn) {
                ...
                revealCorrect(q.type, isCorrect ? null : btn);
                ...
                if (isCorrect) {
                    ...
                    feedbackText.textContent = describeAngle(q);
            ...
            function handleTimeout() {
                ...
                lockAnswers();
                const q = session.questions[session.index];
                revealCorrect(q.type, null);
                feedbackBanner.classList.remove('d-none');
                feedbackText.textContent = "Time's up! " + describeAngle(q);
                feedbackBanner.classList.add('wrong-banner');
                playWrongSound();
                continueBtn.classList.remove('d-none');
            }
            ...
            function showResults() {
                ...
                if (pct >= 0.9) message = "Amazing! You're an angles superstar!";
