...
@section('meta_title', 'Ratio & Proportion — GCSE Maths Game')
@section('meta_blurb', 'A free ratio and proportion game for GCSE-level practice — share amounts in a given ratio, and scale quantities using direct proportion.')
@section('meta_words', 'ratio game, proportion game, share in a ratio, direct proportion, gcse maths game, learn ratio')
...
@push('styles')
    <style>
        ...
        @media (max-width: 400px) {
            .ratio-question { font-size: 1.25rem; }
        }
...
@section('content')
    <div class="container-xl pb-2">
                ...
                <div class="card mult-setup-card">
                    <div class="card-body p-4">
                        ...
                        <div class="mb-4" id="typeToggles">
                            <button type="button" class="choice-toggle active" data-type="share">Share a total in a ratio</button>
                            <button type="button" class="choice-toggle active" data-type="proportion">Direct proportion</button>
                        </div>

                        <h2 class="h6 text-uppercase text-secondary mb-3"><i class="bi bi-grid-3x3-gap"></i> Which ratio numbers?</h2>
                        ...
                        <div id="setupError" class="alert alert-warning mt-4 mb-0 d-none" role="alert">
                            Pick at least one question type and one ratio number to play with.
                        </div>
                ...
                <div class="question-card-wrap mb-3">
                    <div class="card question-card">
                        <div class="card-body p-4 text-center">
                            <span class="target-badge" id="targetBadge">SHARE IN A RATIO</span>
                            <div class="ratio-question" id="ratioQuestion">Share £32 in the ratio 3 : 5. What is the first share?</div>
                        </div>
                    </div>
        ...
        <div class="mult-wrap mt-5">
            <h2 class="h6 text-secondary text-uppercase mb-2">About this ratio and proportion game</h2>
            <p class="text-secondary small">This free ratio and proportion game gives GCSE-level students practice sharing an amount in a given ratio, and scaling quantities using direct proportion. Choose which question types and ratio numbers to include, set your question count and time limit, then work through as many as you can. It's a solid way to build fluency with two of the most common exam question styles.</p>
        </div>
    </div>
@endsection
...
@push('scripts')
    <script>
        (function() {
            ...
            const tableToggles = Array.from(document.querySelectorAll('#tableToggles .table-toggle'));
            ...
            const ratioQuestion = document.getElementById('ratioQuestion');
            ...
            const ALL_TYPES = ['share', 'proportion'];
            ...
            function getHint(q) {
                if (q.type === 'share') return 'Add the ratio parts together, divide the total by that sum, then multiply by the part you need.';
                return 'Work out the price of ONE item first, then multiply by how many you need.';
            }

            function loadSettings() {
                try {
                    const raw = localStorage.getItem('ratioProportionGame.settings');
                    if (!raw) return;
            ...
            function saveSettings() {
                try {
                    localStorage.setItem('ratioProportionGame.settings', JSON.stringify({
            ...
            pickAllBtn.addEventListener('click', function() {
                selectedNumbers = new Set(ALL_NUMBERS);
                setupError.classList.add('d-none');
                renderToggles();
            });
            ...
            function buildQuestion(types, numbers) {
                ...
                if (type === 'share') {
                    let a = pickFrom(numbers), b = pickFrom(numbers);
                    const k = randInt(2, 10);
                    const total = k * (a + b);
                    const askFirst = Math.random() < 0.5;
                    const answer = askFirst ? k * a : k * b;
                    const which = askFirst ? 'first' : 'second';
                    return {
                        type: type,
                        display: 'Share £' + total + ' in the ratio ' + a + ' : ' + b + '. What is the ' + which + ' share?',
                        answer: answer,
                        a: a, b: b, total: total, k: k,
                        badge: 'SHARE IN A RATIO',
                    };
                }

                // proportion — direct proportion, "if X cost £C, how much do Y cost"
                const unitPrice = randInt(2, 20);
                const x = pickFrom(numbers);
                const y = pickFrom(numbers);
                const cost = unitPrice * x;
                const answer = unitPrice * y;
                return {
                    type: type,
                    display: 'If ' + x + ' items cost £' + cost + ', how much would ' + y + ' items cost?',
                    answer: answer,
                    x: x, y: y, cost: cost, unitPrice: unitPrice,
                    badge: 'DIRECT PROPORTION',
                };
            }
            ...
            function buildQuestions(types, numbers, count) {
                ...
                for (let i = 0; i < count; i++) {
                    ...
                    do {
                        ...
                        key = q.type === 'share' ? ('share:' + q.a + ':' + q.b + ':' + q.k) : ('proportion:' + q.x + ':' + q.y + ':' + q.unitPrice);
                        attempts++;
            ...
            function buildChoices(q) {
                ...
                if (q.type === 'share') {
                    const other = q.total - q.answer;
                    if (other > 0 && other !== answer) choices.add(other);
                }
            ...
            function showQuestion() {
                ...
                ratioQuestion.textContent = q.display;
            ...
            function revealCorrect(correctAnswer, chosenBtn) {
                Array.from(answerGrid.querySelectorAll('.answer-btn')).forEach(function(btn) {
                    if (parseInt(btn.textContent.replace('£', ''), 10) === correctAnswer) {
            ...
            function feedbackForWrong(q) {
                if (q.type === 'share') {
                    return q.a + ' + ' + q.b + ' = ' + (q.a + q.b) + '; £' + q.total + ' ÷ ' + (q.a + q.b) + ' = £' + (q.total / (q.a + q.b)) + ' per part; answer = £' + q.answer;
                }
                return '£' + q.cost + ' ÷ ' + q.x + ' = £' + q.unitPrice + ' each; £' + q.unitPrice + ' × ' + q.y + ' = £' + q.answer + '.';
            }
            ...
            function handleAnswer(choice, btn) {
                ...
                if (isCorrect) {
                    ...
                    feedbackText.textContent = feedbackForWrong(q);
            ...
            function handleTimeout() {
                ...
                feedbackBanner.classList.remove('d-none');
                feedbackText.textContent = "Time's up! " + feedbackForWrong(q);
                feedbackBanner.classList.add('wrong-banner');
                playWrongSound();
                continueBtn.classList.remove('d-none');
            }
            ...
            function showResults() {
                ...
                if (pct >= 0.9) message = "Amazing! You're a ratio superstar!";
            ...
            if (selectedTypes.size === 0) {
                selectedTypes = new Set(['share', 'proportion']);
            }
            if (selectedNumbers.size === 0) {
                selectedNumbers = new Set([2, 3, 4, 5, 6]);
            }
